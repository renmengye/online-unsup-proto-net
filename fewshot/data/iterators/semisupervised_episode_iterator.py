...
class SemiSupervisedEpisodeIterator(EpisodeIterator):
  ...
  def process_one(self, collection):
    ...
    def query_tf(x):
      ...
      x = x_valid * x + (1 - x_valid) * self.unk_id
    ...
    lbl_s_masked[unlabeled] = self.unk_id
    ...
    lbl_s_gt[0] = self.unk_id
    # Labeled to be trained as target.
    cond = lbl_s_np[1:] > cummax[:-1]
    lbl_s_gt[1:] = np.where(cond, self.unk_id, lbl_s_np[1:])
