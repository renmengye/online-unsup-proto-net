...
@RegisterIterator('mix-unsupervised-augment-sim-episode')
class MixUnsupAugmentSimEpisodeIterator(SimEpisodeIterator):
  ...
  def __init__(self,
    ...
    super(MixUnsupAugmentSimEpisodeIterator, self).__init__(
        dataset,
        config,
        None,
        batch_size,
  ...
  def get_dataset(self):
    ...
    def simclr_preprocess(data):
      ...
      return {'x': tf.concat([data['x'], x], axis=-1)}
    ...
    ds = ds.batch(self.config.maxlen)
    ds = ds.map(preprocess)

    def form_episode(data):
      T = tf.shape(data['x'])[0]
      if self._data_format2 == "NCHW":
        x = tf.transpose(data['x'], [0, 3, 1, 2])

      return {
          'x_s': x[None],  # Expand axis.
          'y_s': tf.zeros([1, T]) + self._unk_id,
          'y_gt': tf.zeros([1, T]) + self._unk_id,
          'y_full': tf.zeros([1, T]) + self._unk_id,
          'flag_s': tf.ones([1, T])
      }

    ds = ds.map(form_episode)
    if self._prefetch:
  ...
  def get_generator(self):
    ...
    while True:
      ...
      # print('multi', self._rank, self._totalrank, N, start, end)
      self._reset_flag = False
