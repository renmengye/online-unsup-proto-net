...
@RegisterIterator('unsupervised-augment-sim-episode-multigpu')
class UnsupervisedAugmentSimEpisodeIteratorMultiGPU(SimEpisodeIterator):
  ...
  def __init__(self,
    ...
    self._simclr_proc = SIMCLRPreprocessor(
        ...
        map_fn=self._map_fn)
    super(UnsupervisedAugmentSimEpisodeIteratorMultiGPU, self).__init__(
  ...
  def process_one(self, collection):
    ...
    """
    # print('rank', self._rank, '/', self._totalrank, 'ID', collection, 'seed',
    #       self._seed)
    if self._rank > 3:
      # log.error('Maximum rank per episode: 4')
      idx = (collection + 1) % len(self.dataset)
    else:
      idx = collection
    episode = super(UnsupervisedAugmentSimEpisodeIteratorMultiGPU,
    ...
    # Get rank specific chunk.
    if self._totalrank > 1:
      ll = x_s.shape[0]
      MAX_RANK = 4
      if self._totalrank > MAX_RANK:
        # log.error('Maximum rank per episode: 4')
        m = ll // MAX_RANK
      else:
        m = ll // self._totalrank

      rank = self._rank % MAX_RANK
      start2 = rank * m
      end2 = (rank + 1) * m
      x_s = x_s[start2:end2]
      x_att = x_att[start2:end2]
      y_full = y_full[start2:end2]
      flag_s = flag_s[start2:end2]
    ...
    epi = {
        'x_s': img_all,
        'y_s': tf.zeros_like(y_full) + self._unk_id,
        'y_gt': tf.zeros_like(y_full) + self._unk_id,
        'y_full': y_full,
        'flag_s': flag_s,
        'id': idx
    }
