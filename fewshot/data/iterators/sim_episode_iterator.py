...
class SimEpisodeIterator(Iterator):
  ...
  def __init__(self,
    ...
    self._max_bbox_per_object = 9
    self._transform_bbox = True
    self._data_format = data_format
    # sampler.set_num(dataset.get_size())
    if sampler is not None:
      sampler.set_dataset(dataset)
    self._batch_size = batch_size
    self._nclasses = config.nway
    ...
    self._max_num_per_cls = config.max_num_per_cls
    self._seed = seed
    self._rnd = np.random.RandomState(seed)
    self._preprocessor = preprocessor
    self._maxlen = config.maxlen
    self._prefetch = prefetch
    self._semisupervised = config.semisupervised
    self._unsupervised = config.unsupervised
    self._cat_dict = {}
    self._label_ratio = config.label_ratio
  ...
  def process_one(self, idx):
    ...
    y_abs = np.zeros([N], dtype=np.int32)
    ...
    for item in raw:
      ...
      if len(keys) > 0:
        ...
        for key in keys:
          ...
          # Skip if there is no segmentation map.
          ...
          if obj['category'] not in self._cat_dict:
            self._cat_dict[obj['category']] = len(self._cat_dict)
          y_abs[fcount] = inst_id + self._cat_dict[obj['category']] * 100000
          flag[fcount] = 1
    ...
    y_abs = slice(y_abs, start, start + T, reverse)
    ...
    #   y_s = y_s[start2:end2]
    #   bboxes = bboxes[start2:end2]
    #   categories = categories[start2:end2]
    #   instance_ids = instance_ids[start2:end2]
    #   flag = flag[start2:end2]
    #   # print(self._rank, idx, start2, end2)
    ...
    if self._semisupervised:
      ...
      y_s2[unlabeled] = -1
    elif self._unsupervised:
      labeled = np.zeros_like(y_s).astype(np.bool)
      unlabeled = np.logical_not(labeled)
      y_s_orig = np.copy(y_s)
      y_s = np.zeros_like(y_s) + unk_id
      y_s2 = np.zeros_like(y_s) - 1
    else:
      # Recompute y_s
    ...
    if self._data_format == "NCHW":
      x_s = tf.transpose(x_s, [0, 3, 1, 2])
      x_att = tf.transpose(x_att, [0, 3, 1, 2])
    episode = {
  ...
  def get_dataset(self):
    ...
    def preprocess(data):
      ...
      if self._data_format == "NCHW":
        ...
        axis = -1
      x_s = self.preprocessor(data['x_s'])
      x_att = tf.image.convert_image_dtype(data['x_att'], tf.float32)
      data['x_s'] = tf.concat([x_s, x_att], axis=axis)
      del data['x_att']
      return data
