...
class MixUnsupAugmentSimIteratorNonIIDV2(SimEpisodeIterator):
  ...
  def __init__(self,
    ...
    batch_size = batch_size * totalrank
    super(MixUnsupAugmentSimIteratorNonIIDV2, self).__init__(
        dataset,
        config,
        None,
        batch_size,
  ...
  def process_one(self, idx):
    ...
    """
    # print('read data', self._rank, idx)
  ...
  def get_dataset(self):
    ...
    M = self._totalrank
    cur_batch_size = self.batch_size // M
    print('total batch size', self.batch_size)
    print('cur batch size', cur_batch_size)

    def _get_cur_batch(data):
      ll = data['x'].shape[0]
      m = ll // self._totalrank
      start2 = self._rank * m
      end2 = (self._rank + 1) * m
      return {'x': data['x'][start2:end2], 'bbox': data['bbox'][start2:end2]}

    ds = ds.batch(self.batch_size).map(
        _get_cur_batch, deterministic=True).unbatch()
    ...
    ds = ds.batch(cur_batch_size)

    # def _get_cur_batch2(data):
    #   return {'x': data['x'][start2:end2]}

    # ds = ds.batch(self.batch_size).map(_get_cur_batch2, deterministic=True)
  ...
  def __len__(self):
    ...
    size = np.floor(N) / float(self.batch_size)
