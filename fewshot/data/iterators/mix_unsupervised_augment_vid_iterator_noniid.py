...
class MixUnsupAugmentVidIteratorNonIID(UnsupervisedAugmentVidEpisodeIterator):
  ...
  def get_generator(self):
    ...
    while True:
      ...
      self._reset_flag = False
      T2 = self.config.frame_rate * self.config.maxlen
      # print("hey", self.dataset._len_per_file, T2, self.config.frame_rate,
      #       self.config.maxlen)
      msg = "File length too short: {} < {}".format(self.dataset._len_per_file,
                                                    T2)
      assert self.dataset._len_per_file > T2, msg
      # T = min(T2, self.dataset._len_per_file - T2)
      T = T2
  ...
  def get_dataset(self):
    ...
    def truncate(data):
      if self._totalrank > 1:
        ...
        end2 = (self._rank + 1) * m
        for k in data:
          data[k] = data[k][start2:end2]
        return data
      else:
        return data

    ds = ds.map(truncate, deterministic=True)
    ds = ds.interleave(
        tf.data.Dataset.from_tensor_slices, cycle_length=1, deterministic=True)
    ...
    ds = ds.batch(self.config.maxlen // self._totalrank)
