...
# from tensorflow.python.ops.variables import validate_synchronization_aggregation_trainable # NOQA
...
class HierarchicalEpisodeSampler(EpisodeSampler):
  ...
  def sample_episode_classes(self,
    ...
    """
    cls_list = []
    assert np.mod(n, nstage) == 0, "{}, {}".format(n, nstage)
    n_ = n // nstage
    if max_num == -1:
      max_num_ = -1
    else:
  ...
  def sample_classes(self, n, save_additional_info=False, **kwargs):
    ...
    if not self.use_class_hierarchy:
      ...
      if not self.use_new_class_hierarchy:
        ...
        #   category_str = []
        #   for i, c in enumerate(np.unique(episode_classes)):
        #     category_str.append(
        #         self._dataset.label_general_str[hmap[c % len(hmap)]])

        for c, s in zip(episode_classes, stage):
          # Magic number is 3.
          # print(len(self._dataset.label_str))
          # print('c', c, 's', s,
          # self._dataset.label_general_str[hmap[c % len(hmap)]],
          # self.hierarchy_dict[hmap[c % len(hmap)]][s])

          # print(self.hierarchy_dict)
          results.append(self.hierarchy_dict[hmap[c % len(hmap)]][s])
          # print('s = {} {}'.format(s, self._dataset.label_general_str[s]),
          #       'c = {}'.format(c))
      ...
      if save_additional_info:
        ...
        if category_str is not None:
          info['category_str'] = np.array(category_str)
        return results, info
      else:
        return results
