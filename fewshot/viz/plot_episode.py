...
def plot_one_episode(episode,
  ...
  lbl_full = episode['y_full'].numpy()
  print('lbl', lbl)
  print('lbl gt', lbl_gt)
  # category_str = episode['category_str']
  ...
  if 'stage_id' in episode:
    ...
    print('class id in stage', class_id_in_stage)
  else:
    is_hierarchical = False
    stage_id = None
    class_id_in_stage = None
  ...
  for i in range(1, 1 + min(ncol * nrow, img.shape[1])):
    ...
    border_w = img_i.shape[1] - 1 if img_i.shape[1] <= 28 else img_i.shape[1]
    border_h = img_i.shape[0] - 1 if img_i.shape[0] <= 28 else img_i.shape[0]
    rect = patches.Rectangle((0, 0),
                             border_w,
                             border_h,
                             linewidth=max(int(img_i.shape[1] / 25.0), 2),
                             edgecolor=edgecolor,
    ...
    ax.axis('off')
    # title = '{}'.format(i - 1)
    title = '{}'.format(lbl_full_i)
    ax.set_title(title)
    # cmap = np.array([[61, 220, 151], [70, 35, 122], [37, 110, 255],
    #                  [216, 216, 244], [139, 148, 163]]) / 255.0
    cmap = np.array([[33, 145, 201], [0, 169, 177], [240, 132, 119],
                     [255, 230, 153], [139, 148, 163]]) / 255.0
    stagename = ['A', 'B', 'C', 'D', 'E']
    # stage_text = ['black', 'white', 'white', 'black', 'white']
    stage_text = ['white', 'white', 'white', 'black', 'white']

    # Color box for stage.
    if is_hierarchical and False:
      stage_i = stage_id[0, i - 1]
      class_i = class_id_in_stage[0, i - 1]
      stage_f = float(stage_i) / stage_id.max()
      stage_color = cmap[stage_i]
      rect_w = 12 if img_i.shape[1] == 28 else 32
      rect_h = 11 if img_i.shape[1] == 28 else 28
      ...
      ax.add_patch(rect)
      ax.text(
          int(img_i.shape[1] / 18.6),
          int(img_i.shape[1] / 4.6),
...
def plot_episodes(data_it, nepisode, fname_tmpl, **kwargs):
  ...
  """
  # for i, data in zip(range(nepisode), data_it):
  #   for j, e in zip([0], data):
  #     plot_one_episode(e, fname_tmpl.format(i), **kwargs)
  for i, data in zip(range(nepisode), data_it):
    plot_one_episode(data, fname_tmpl.format(i), **kwargs)
...
def main():
    ...
    if args.stage:
      data = [
          get_dataiter_continual(
              ...
              stage=s) for s in range(args.nepisode)
      ]
    else:
  ...
  if data_config.maxlen <= 40:
    ...
    ncol = 5
    nrow = data_config.maxlen // ncol
  else:
    ncol = 6
    nrow = 25

  # Plot episodes.
  # plot_episodes([data[i]['val_fs'] for i in range(args.nepisode)],
  #               args.nepisode,
  ...
  plot_episodes(
      data['test_fs'],
      args.nepisode,
      os.path.join(args.output, 'test', 'episode_{:06d}'),
      model=fs_model,
      nway=data_config.nway,
      ncol=ncol,
      nrow=nrow,
