...
class LARSOptimizer(tf.train.Optimizer):
  ...
  def get_weights(self):
    results = []
    keys = sorted(self._weights.keys())
    for k in keys:
      # print('getweights', k)
      results.append(self._weights[k].numpy())
    return results
  ...
  def apply_gradients(self, grads_and_vars, global_step=None, name=None):
    ...
    for (grad, param) in grads_and_vars:
      ...
      v = get_variable(
          param_name + "/Momentum",
          initializer=lambda: tf.zeros(param.shape.as_list()),
          dtype=tf.float32,
          trainable=False)
      if v.name not in self._weights:
        # print('add weights', v)
        self._weights[v.name] = v

      if self._use_weight_decay(param_name):
        grad += self.weight_decay * param
        log.info('Apply weight decay {}: {:.1e}'.format(
            param.name, self.weight_decay))
      ...
      if self.classic_momentum:
        ...
        if self._do_layer_adaptation(param_name):
          ...
          trust_ratio = tf.where(
              tf.greater(w_norm, 0),
              tf.where(
                  tf.greater(g_norm, 0), (self.eeta * w_norm / g_norm), 1.0),
              1.0)
        ...
        if self._do_layer_adaptation(param_name):
          ...
          trust_ratio = tf.where(
              tf.greater(w_norm, 0),
              tf.where(
                  tf.greater(v_norm, 0), (self.eeta * w_norm / v_norm), 1.0),
              1.0)
