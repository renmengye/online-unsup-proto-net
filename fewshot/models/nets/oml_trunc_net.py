...
class OMLTruncSigmoidNet(EpisodeRecurrentSigmoidNet):
  ...
  def __init__(self,
               ...
               distributed=False,
               dtype=tf.float32):
    super(OMLTruncSigmoidNet, self).__init__(config,
                                             backbone,
  ...
  def compute_loss(self, x, y, dt, y_gt, flag, *states, **kwargs):
    ...
    logits, states = self.forward_partial(x,
                                          y,
                                          dt,
                                          *states,
    ...
    xent, xent_unk = self.xent_with_unk(logits_all,
                                        labels_all,
                                        K,
                                        flag=flag_all)
      ...
      for t_start in tf.range(0, T, DT):
        ...
        with tf.GradientTape() as tape:
          ...
          loss, metric, states = self.compute_loss(
              ...
              y_gt[:, t_start:t_end], flag[:,
                                           t_start:t_end], *states, **kwargs)
        ...
        flag_total_ = tf.reduce_sum(tf.cast(flag[:, t_start:t_end],
                                            self.dtype))
  ...
  def eval_step(self, x, y, **kwargs):
    ...
    """
    logits, _ = self.forward_partial(x,
                                     y,
                                     tf.shape(x)[1],
...
class OMLTruncNet(EpisodeRecurrentNet):
  ...
  def __init__(self,
               ...
               distributed=False,
               dtype=tf.float32):
    super(OMLTruncNet, self).__init__(config,
                                      backbone,
                                      distributed=distributed,
                                      dtype=dtype)
    assert config.fix_unknown, 'Only unknown is supported'
