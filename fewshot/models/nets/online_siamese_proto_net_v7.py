...
@RegisterModel("online_siamese_proto_net_v7")
class OnlineSiameseProtoNetV7(OnlineProtoNet):
  ...
  def __init__(self,
               ...
               distributed=False,
               dtype=tf.float32):
    super(OnlineSiameseProtoNetV7, self).__init__(
  ...
  def forward(self,
    ...
    if is_training:
      ...
      if self.backbone.config.data_format == 'NCHW':
        x_list = tf.split(x, 2, axis=2)  # M+1 x [B, T, C, H, W]
      else:
        x_list = tf.split(x, 2, axis=-1)  # M+1 x [B, T, H, W, C]
      x = tf.concat(x_list, axis=0)

    h = self.run_backbone(x, is_training=is_training)  # [3xB, D]
    h = self.run_head(h, self._projector, is_training=is_training)
    if is_training:
      h1 = h[:B]  # [B, T, D] # Use the first dim
      h2 = h[B:]  # [B, T, D]

    y_pred = tf.TensorArray(self.dtype, size=T)
    y_unk = tf.TensorArray(self.dtype, size=T)
    y_pred1 = tf.TensorArray(self.dtype, size=T)
    y_pred_lb1 = tf.TensorArray(self.dtype, size=T)
    y_pred2 = tf.TensorArray(self.dtype, size=T)
    y_pred_lb2 = tf.TensorArray(self.dtype, size=T)
    if len(states) == 0:
    ...
    for t in tf.range(T):
      ...
      # ---------------- Self-supervision ----------------
      if is_training:
        y_pred_lb1_ = self.memory.retrieve(h1_, t, *states_new1, write=True)
        y_pred_lb1 = y_pred_lb1.write(t, y_pred_lb1_)  # [B, K]
        y_pred1_ = self.memory.retrieve(h2_, t, *states_new1, write=True)
        y_pred1 = y_pred1.write(t, y_pred1_)

        y_pred_lb2_ = self.memory.retrieve(h2_, t, *states_new2, write=True)
        y_pred_lb2 = y_pred_lb2.write(t, y_pred_lb2_)  # [B, K]
        y_pred2_ = self.memory.retrieve(h1_, t, *states_new2, write=True)
        y_pred2 = y_pred2.write(t, y_pred2_)

      # Select valid states.
      states = tuple([
          tf.where(_expand(tf.cast(flag[:, t], tf.bool), s), s_new, s)
          for s, s_new in zip(states, states_new1)
      ])
    ...
    if is_training:
      # [T, M, B, K] -> [B, M, T, K]
      y_pred1 = tf.transpose(y_pred1.stack(), [1, 0, 2])  # [B, T, K]
      y_pred2 = tf.transpose(y_pred2.stack(), [1, 0, 2])  # [B, T, K]
      y_pred_lb1 = tf.transpose(y_pred_lb1.stack(), [1, 0, 2])  # [B, T, K]
      y_pred_lb2 = tf.transpose(y_pred_lb2.stack(), [1, 0, 2])  # [B, T, K]
      y_pred_lb1 = tf.stop_gradient(y_pred_lb1)  # [B, T, K]
      y_pred_lb2 = tf.stop_gradient(y_pred_lb2)  # [B, T, K]
      y_pred12 = tf.concat([y_pred1, y_pred2], axis=0)
      y_pred_lb12 = tf.concat([y_pred_lb1, y_pred_lb2], axis=0)
      flag_all = tf.concat([flag, flag], axis=0)
      loss, metric = self.compute_ssup_loss(y_pred, y_unk, y_pred_lb12,
                                            y_pred12, flag_all)
      ...
      metric = {}
    return y_pred, y_pred_lb1, states, loss, metric
  ...
  def compute_ssup_loss(self, pred, pred_unk, labels, logits, flag):
    ...
    if self.config.memory_net_config.soft_label:
      ...
      labels_old = tf.one_hot(
          tf.argmax(labels_old, axis=-1), K, axis=-1)  # [B, MT, K]
    ...
    y_hard = tf.one_hot(
        tf.argmax(pred_old, axis=-1), tf.shape(pred)[-1] - 1,
        axis=-1)  # [B, T, K]
