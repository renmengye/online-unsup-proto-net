...
Allows multiple GPUs in parallel.
...
@RegisterModel("online_siamese_proto_net_multigpu")
class OnlineSiameseProtoNetMultiGPU(OnlineProtoNet):
  ...
  def __init__(self,
               ...
               distributed=False,
               dtype=tf.float32):
    super(OnlineSiameseProtoNetMultiGPU, self).__init__(
  ...
  def run_head(self, x, head, is_training=tf.constant(True)):
    ...
    """
    assert self.config.memory_net_config.cluster_projection_nlayer == 0
    proj_nlayer = self.config.memory_net_config.cluster_projection_nlayer
    if proj_nlayer > 0:
      x_shape = tf.shape(x)
  ...
  def forward(self,
    ...
    if self._distributed:
      # Synchonize all states.
      # No gradients to past states anyways.
      states = tuple([hvd.broadcast(s, 0) for s in states])
    ...
    T = tf.shape(x)[1]
    if self._distributed:
      T = T * hvd.size()  # Longer sequence with distributed.
    ...
    if is_training:
      ...
      x = tf.concat(x_list, axis=0)  # [MxB, T, H, W, C]

    # ------ REGULAR VERSION -------
    h = self.run_backbone(x, is_training=is_training)  # [1, 2xB, T, D]

    # Gather here.
    if self._distributed:
      h = h[None]
      h_gather = hvd.allgather(h)  # [2, 2xB, T, D]
      h_gather = tf.transpose(h_gather, [1, 0, 2, 3])  # [2xB, 2, T, D]

      # [2xB, NT, D]
      h = tf.reshape(h_gather,
                     [tf.shape(h_gather)[0], -1,
                      tf.shape(h_gather)[3]])
      flag = tf.reshape(
          tf.transpose(hvd.allgather(flag[None]), [1, 0, 2]), [B, T])
      y = tf.reshape(tf.transpose(hvd.allgather(y[None]), [1, 0, 2]), [B, T])
    ...
    if is_training:
      ...
      # print("flag", flag.shape, M, T)
      flag_all = tf.reshape(tf.tile(flag[:, None, :], [1, M, 1]), [B, M * T])
      loss, metric = self.compute_ssup_loss(y_pred, y_pred_lb, y_pred_aug,
                                            flag_all)
  ...
  def compute_ssup_loss(self, pred, labels, logits, flag):
    ...
    pred_old = pred[:, :, :-1]
    pred_new = pred[:, :, -1]
    new_prob0 = tf.math.sigmoid(pred_new)
    ...
    # new_prob = new_prob0
    T_ = tf.cast(tf.reduce_min(flag_sum), tf.int64)
    new_prob_max = tf.reduce_max(new_prob[:, :T_])  # []
    new_prob_min = tf.reduce_min(new_prob[:, :T_])  # []
    ...
    alpha = 4.0 * target_mean
    beta = 4.0 * (1.0 - target_mean)
    # alpha = 8.0 * target_mean
    # beta = 8.0 * (1.0 - target_mean)
    nc_loss = -tfp.distributions.Beta(alpha, beta).log_prob(new_prob)
