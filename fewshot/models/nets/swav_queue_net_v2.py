...
@RegisterModel("swav_queue_net_v2")
class SWAVQueueNetV2(Net):
  ...
  def __init__(self,
               ...
               distributed=False,
               dtype=tf.float32):
    super(SWAVQueueNetV2, self).__init__()
    ...
    queue = BatchStorage(1, Q, [D])
    self._queue_var = self._get_variable(
        "queue_var", lambda: queue.all, trainable=False)
    # self._queue_var2 = self._get_variable(
    #     "queue_var2", lambda: queue.all, trainable=False)
  ...
  def _get_optimizer(self, optname, learn_rate):
    if optname == 'lars':
      ...
      return super(SWAVQueueNetV2, self)._get_optimizer(optname, learn_rate)
  ...
  def loss_hidden(self, hidden1, hidden2):
    ...
    Qsize = self.config.contrastive_net_config.queue_size
    warmup_step = self.config.contrastive_net_config.queue_warmup_steps
    # print(self._queue_var)
    queue = BatchStorage.from_states(self._queue_var)
    hidden_q = tf.reshape(queue.all, [Qsize, -1])
    warmup_step = self.config.contrastive_net_config.queue_warmup_steps
    temperature = self.config.contrastive_net_config.temperature

    # Gather hidden1/hidden2 across replicas and create local labels.
    if self._distributed:
      hidden1_large = hvd.allgather(hidden1, name='hidden1_large')
    else:
      hidden1_large = hidden1
    #   hidden2 = hvd.allgather(hidden2, name='hidden2')

    # if self._step > warmup_step:
    #   hidden1q = tf.concat([hidden_q, hidden1], axis=0)  # [Q+T, D]
    #   hidden2q = tf.concat([hidden_q, hidden2], axis=0)  # [Q+T, D]
    # else:
    #   hidden1q = hidden1
    #   hidden2q = hidden2

    # if self._distributed:
    #   hidden1_root = hvd.broadcast(hidden1, 0, name='hidden1_root')
    #   tf.print('hidden1_root', hvd.rank(), tf.reduce_mean(hidden1_root))
    # else:
    #   hidden1_root = hidden1
    ...
    if self._step > warmup_step:
      score_q = tf.matmul(hidden_q, self._prototypes)
      score1_large = tf.concat([score_q, score1_large], axis=0)
      score2_large = tf.concat([score_q, score2_large], axis=0)
    ...
    if self._step >= warmup_step:
      q1 = q1[Qsize:]
      q2 = q2[Qsize:]
      logits1 = logits1[Qsize:]
      logits2 = logits2[Qsize:]

    # T = self.global_batch_size
    T = tf.shape(hidden1_large)[0]
    B = 1
    ...
    item_idx = tf.reshape(
        tf.tile(
            tf.reshape(
                tf.math.mod(
                    tf.range(self._step * T64, (self._step + 1) * T64), Qsize),
                [1, -1]), [B, 1]), [-1])
    b_idx = tf.reshape(
        tf.tile(tf.reshape(tf.range(B, dtype=tf.int64), [-1, 1]), [1, T]),
        [-1])
    key_batch = tf.stop_gradient(tf.stack([b_idx, item_idx], axis=-1))
    queue.setitem_batch(key_batch, tf.stop_gradient(hidden1_large))  # [BT, 2]
    self._queue_var.assign(queue.get_states())
  ...
  def regularized_weights(self):
    ...
    rw = super(SWAVQueueNetV2, self).regularized_weights()
  ...
  def train_step(self, x, writer=None, **kwargs):
    ...
    with writer.as_default() if writer is not None else dummy_context_mgr():
      ...
      # Sync prototypes.
      # hvd.broadcast_variables([self._prototypes], 0)
