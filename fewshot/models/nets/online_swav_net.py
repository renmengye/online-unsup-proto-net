...
@RegisterModel("online_swav_net")
class OnlineSwavNet(EpisodeRecurrentSigmoidNet):
  ...
  def __init__(self,
               ...
               distributed=False,
               dtype=tf.float32):
    super(OnlineSwavNet, self).__init__(
  ...
  def regularized_weights(self):
    ...
    rw = super(OnlineSwavNet, self).regularized_weights()
  ...
  def forward(self,
    ...
    if is_training:
      ...
      ssup_label = tf.concat([q2, q1], axis=1)  # [B, MT]
      ssup_pred = tf.concat([logits1, logits2], axis=1)  # [B, MT]
  ...
  def eval_step(self, x, y, **kwargs):
    ...
    r = super(OnlineSwavNet, self).eval_step(x, y, **kwargs)
    self.memory.clear_state()
    return r
