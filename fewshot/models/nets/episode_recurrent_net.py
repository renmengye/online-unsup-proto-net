...
class EpisodeRecurrentNet(Net):
  ...
  def __init__(self, config, backbone, distributed=False, dtype=tf.float32):
    ...
    if opt_config.learn_rate_schedule == "cosine":
      self._learn_rate = tf.compat.v1.train.cosine_decay(
          opt_config.lr_list[0], self.step, opt_config.max_train_steps)
    elif opt_config.learn_rate_schedule == "staircase":
  ...
  def apply_gradients(self, loss, tape, opt=None, add_step=tf.constant(True)):
