...
class Net(ContainerModule):
  ...
  def _get_optimizer(self, optname, learn_rate):
    ...
    if optname == 'adam':
      ...
      opt = tf.optimizers.SGD(learning_rate=learn_rate, momentum=0.0, **kwargs)
      ...
      return LARSOptimizer(
          ...
          name="LARSOptimizer",
          exclude_from_weight_decay=['bn', 'batch_norm', '/b'])
    else:
  ...
  def reload(self, **kwargs):
    if self._load_path is not None:
      self.load(self._load_path, **kwargs)

  def load(self, path, load_optimizer=False, verbose=True):
    """Loads the weights."""
    self._load_path = path
    if verbose:
    ...
    if verbose:
      [
          log.info('Weights {} {}'.format(w.name, w.shape))
  ...
  def var_to_optimize(self):
    ...
    for v in self._var_to_optimize:
      log.info("Trainable weight {} {}".format(v.name, v.shape))
