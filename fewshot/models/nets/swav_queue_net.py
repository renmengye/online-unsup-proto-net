...
@RegisterModel("swav_queue_net")
class SWAVQueueNet(Net):
  ...
  def __init__(self,
               ...
               distributed=False,
               dtype=tf.float32):
    super(SWAVQueueNet, self).__init__()
  ...
  def _get_optimizer(self, optname, learn_rate):
    if optname == 'lars':
      ...
      return super(SWAVQueueNet, self)._get_optimizer(optname, learn_rate)
  ...
  def loss_hidden(self, hidden1, hidden2):
    ...
    if self._distributed:
      hidden1 = hvd.allgather(hidden1, name='hidden1')
    ...
    q1 = self.sinkhorn(score1)
    q2 = self.sinkhorn(score2)
    ...
    # print(key_batch, hidden1_all)
    # tf.print('key', key_batch)
    # tf.print('items before', queue.all[0])
    queue.setitem_batch(key_batch, tf.stop_gradient(hidden1))  # [BT, 2]
    # tf.print('items after', queue.all[0])
  ...
  def regularized_weights(self):
    ...
    rw = super(SWAVQueueNet, self).regularized_weights()
