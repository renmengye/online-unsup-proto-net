...
class CPMSSL(ContainerModule):
  ...
  @tf.function
  def forward(self, t, x, y, *states, store=False, ssl_store=None, **kwargs):
    D = self._dim
    ...
    y_cls = self.proto_memory.retrieve(
        x_scale, t, storage_scale, count, beta=beta, gamma=gamma, add_new=True)
    ...
    pred_last.set_shape([x.shape[0], pred_last.shape[1]])
