...
class ProtoMemory(ContainerModule):
  ...
  def __init__(self, name, dim, config, dtype=tf.float32):
    ...
    self._use_variables = config.use_variables
    self._name = name
    self._dim = dim
    ...
    if config.use_variables:
      self._MAX_BSIZE = config.max_bsize
      K = self.max_classes
      if config.fix_unknown:
        K += 1

      # assert config.trainable_storage
      with variable_scope(self._name):
        self._storage_var = self._get_variable(
            "storage_var",
            self._get_constant_init([self._MAX_BSIZE, K, self._dim], 0.0),
            trainable=config.trainable_storage)
        self._count_var = self._get_variable(
            "count_var",
            self._get_constant_init([self._MAX_BSIZE, K], 0.0),
            trainable=config.trainable_storage)
  ...
  def forward(self, x, y, t, *states, **kwargs):
    y_ = self.retrieve(x, t, *states)
    states = self.store(x, y, *states)
    if self.use_variables:
      self._storage_var.assign(states[0]), self._count_var.assign(states[1])
      states = (self._storage_var, self._count_var)
    return y_, states
  ...
  def store(self, x, y, *states, **kwargs):
    """Store a new example.
  ...
  def get_initial_state(self, bsize):
    ...
    if self.use_variables:
      bsize = self._MAX_BSIZE

    storage = tf.zeros([bsize, K, self._dim], dtype=self.dtype)
    count = tf.zeros([bsize, K], dtype=self.dtype)

    if self.use_variables:
      self._storage_var.assign(storage)
      self._count_var.assign(count)
      return self._storage_var, self._count_var
    else:
      # assert False, "Only use var!"
  ...
  def use_variables(self):
    """Whether to use variables to store."""
