...
class SemiSupervisedMinDistProtoMemory(SemiSupervisedProtoMemory):
  ...
  def forward(self, x, y, t, *states, ssl_store=None, **kwargs):
    ...
    y_ = self.retrieve(x, t, *states)  # [B, K]
    ...
    if self.use_variables:
      self._storage_var.assign(storage_new), self._count_var.assign(count_new)
      storage_new, count_new = self._storage_var, self._count_var
    else:
  ...
  def retrieve(self, x, t, *states, beta=None, gamma=None, temp=None,
               **kwargs):
    ...
    B = tf.shape(x)[0]
    if tf.less(B, tf.shape(storage)[0]):
      storage = storage[:B]
      count = count[:B]
