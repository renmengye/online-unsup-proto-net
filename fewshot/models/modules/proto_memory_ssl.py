...
class SemiSupervisedProtoMemory(ProtoMemory):
  ...
  def forward(self, x, y, t, storage, count, **kwargs):
    y_ = self.retrieve(x, t, storage, count)
    y_soft = tf.nn.softmax(y_)
    storage, count = self.store(x, y, storage, count, y_soft=y_soft)
    if self.use_variables:
      self._storage_var.assign(storage), self._count_var.assign(count)
      storage, count = self._storage_var, self._count_var
