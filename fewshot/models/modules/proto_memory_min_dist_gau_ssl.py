...
class SemiSupervisedMinDistGAUProtoMemory(SemiSupervisedMinDistProtoMemory):
  ...
  def forward(self, x, y, t, *states, ssl_store=tf.constant(True), **kwargs):

    h_last, count_last = states
    y_ = self.retrieve(x, t, h_last, count_last)  # [B, K]
    ...
    if self._use_ssl_beta_gamma_write:
      ...
      gamma2 = None
    y2_ = self.retrieve(
        x, t, h_last, count_last, beta=beta2, gamma=gamma2)  # [B, K]
    ...
    if self.use_variables:
      self._storage_var.assign(h_new), self._count_var.assign(count_new)
      h_new, count_new = self._storage_var, self._count_var
    else:
      h_new.set_shape([x.shape[0], self.max_classes + 1, self._dim])
      count_new.set_shape([x.shape[0], self.max_classes + 1])
    return y_, (h_new, count_new)

  def store(self, x, y, *states, y_soft=None, **kwargs):
    """Store a new example.
