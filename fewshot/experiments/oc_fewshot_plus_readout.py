...
from fewshot.experiments.get_data_iter import (
    get_dataiter_continual, get_dataiter_epoch, get_dataiter_sim,
    get_dataiter_vid)
...
from fewshot.experiments.utils import (ExperimentLogger, delete_old_ckpt,
                                       get_config, get_data, get_data_fs,
...
def train(model,
  ...
  for i, batch in zip(it, dataiter):
    ...
    if is_chief and ((i + 1) % config.steps_per_save == 0):
      ckpt = os.path.join(ckpt_folder, 'weights-{}'.format(i + 1))
      save_ckpt = os.path.join(final_save_folder, 'weights-{}'.format(i + 1))
      model.save(ckpt)
    ...
    # Evaluate.
    if is_chief and ((i + 1) % config.steps_per_val == 0 or i == start):
      # if is_chief and ((i + 1) % config.steps_per_val == 0):
      # readout_net.load(ckpt)
      readout_acc = eval_fn(i)
      if readout_acc > best_acc:
        model.save(os.path.join(final_save_folder, 'best-{}'.format(i + 1)))
        best_acc = readout_acc
...
def main():
  ...
  readout_classes = config.num_classes
  ...
  if args.always_new:
    log.info('Always new set to True')
    config.memory_net_config.always_new = args.always_new
  ...
  assert not args.eval, (
      "Only used for training. Use eval_contrastive for testing.")
  ...
  # config.optimizer_config.optimizer = "sgd"
  config.num_classes = readout_classes
  readout_net = build_pretrain_net(config, backbone=mem_model.backbone)
  # readout_net = build_pretrain_net(config)
  readout_net._learn_rate = tf.constant(args.readout_lr)
  # Only optimize the last layer.
  readout_net._var_to_optimize = [
      readout_net._fc._weight, readout_net._fc._bias
  ]
  readout_net._optimizer = readout_net._get_optimizer("adam",
                                                      readout_net.learn_rate)
  readout_net._wd = 0.0  # Force no weight decay.
  ...
  if args.readout_v2:
    ...
    # Use adam linear eval.
    assert args.eval_with_relu
...
if __name__ == '__main__':
  ...
  parser.add_argument('--always_new', action='store_true')
  ...
  parser.add_argument('--readout_optimizer', type=str, default="adam")
