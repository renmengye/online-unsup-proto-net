...
def calc_ap(results_list, verbose=True):
  ...
  unk_id = np.max(results_list[0]['y_gt'])
  y_gt_list = []
  y_full_list = []
  pred_list = []
  score_list = []
...
def calc_interval(y_gt, y=None):
  ...
  for i in range(T):
    # print(i, 'last seen', last_seen)
    last_seen_ = last_seen[np.arange(B), y_gt[:, i]]
    ninterval[:, i] = np.where(last_seen_ >= 0, i - last_seen_, 0)
...
def convert_pred_to_full(y_pred):
  # print('before', y_pred)
  unk_flag = y_pred == np.max(y_pred, axis=-1)  # [B, T]
  # print(unk_flag)
  unk = (unk_flag).astype(np.int32)
  # print(unk)
  replace = np.cumsum(unk, axis=-1) - 1  # [B, T]
  # print(replace)
  y_full = y_pred.copy()
  y_full[unk_flag] = replace[unk_flag]
  # print('after', y_full)
  return y_full


def calc_rand(results_list):
  results = []
  for r in results_list:
    # print(r['pred_id'].shape, r['flag'].shape)
    for i in range(r['pred_id'].shape[0]):
      flag = r['flag'][i].astype(np.bool)
      y_full = r['y_full'][i][flag]
      pred_id = r['pred_id'][i][flag]
      pred_id = convert_pred_to_full(pred_id)
      s = sklearn.metrics.adjusted_rand_score(y_full, pred_id)
      results.append(s)
  results = np.array(results)
  return mean(results), stderr(results)


def calc_mutual_info(results_list):
  results = []
  for r in results_list:
    for i in range(r['pred_id'].shape[0]):
      flag = r['flag'][i].astype(np.bool)
      ...
      pred_id = r['pred_id'][i][flag]
      pred_id = convert_pred_to_full(pred_id)
      s = sklearn.metrics.adjusted_mutual_info_score(y_full, pred_id)
      results.append(s)
  results = np.array(results)
  return mean(results), stderr(results)


def calc_homogeneity(results_list):
  results = []
  for r in results_list:
    for i in range(r['pred_id'].shape[0]):
      flag = r['flag'][i].astype(np.bool)
      ...
      pred_id = r['pred_id'][i][flag]
      pred_id = convert_pred_to_full(pred_id)
      s = sklearn.metrics.homogeneity_score(y_full, pred_id)
      results.append(s)
  results = np.array(results)
  return mean(results), stderr(results)


def calc_completeness(results_list):
  results = []
  for r in results_list:
    for i in range(r['pred_id'].shape[0]):
      flag = r['flag'][i].astype(np.bool)
      ...
      pred_id = r['pred_id'][i][flag]
      pred_id = convert_pred_to_full(pred_id)
      s = sklearn.metrics.completeness_score(y_full, pred_id)
      results.append(s)
  results = np.array(results)
  return mean(results), stderr(results)
