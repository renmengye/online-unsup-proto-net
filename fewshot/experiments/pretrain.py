    ...
    for ii, batch in zip(it, dataiter):
      ...
      if (ii + 1) % config.steps_per_log == 0 or ii == 0:
        ...
        if is_chief:
          ...
          if proto_net is not None and dataiter_fs is not None:
            post_fix_dict['fs_acc_v'] = '{:.3f}'.format(
                rtest_fs['acc'] * 100.0)
...
def main():
  ...
  if is_chief and not os.path.exists(save_folder):
...
if __name__ == '__main__':
  ...
  # Use the readout checkpoint generated from `eval_contrastive`.
